...
def run_marissa(list_to_use, prefix, percentage_list):
    """main func to be imported and run everything"""
    counter = collections.Counter(list_to_use)
    # values contain times they occur and we save them as tupe objects for the record trie
    marissa = marisa_trie.RecordTrie("<H", zip(counter.keys(), [(x,) for x in counter.values()]))
    marisa_nucleotide_percentage(marissa, percentage_list)
    marissa.prefixes(prefix)
